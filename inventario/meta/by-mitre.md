# Inventario por MITRE ATT&CK

<!-- AUTOGENERADO por scripts/build_indexes.py. NO editar a mano la tabla. -->

Índice facetado generado desde frontmatter. Cada sección agrupa los
archivos por valor de `by-mitre`.

## (sin MITRE) (6)

- [`artefactos-windows-host`](../08-forense-dfir/analisis-forense/artefactos-windows-host.md) — Artefactos de Windows
- [`hashing-codificacion`](../07-fundamentos/criptografia/hashing-codificacion.md) — Hashing y Codificación
- [`linux-arquitectura-permisos`](../07-fundamentos/sistemas/linux-arquitectura-permisos.md) — Arquitectura y Permisos en Linux
- [`metodologia-picerl`](../08-forense-dfir/incident-response/metodologia-picerl.md) — Metodología PICERL
- [`modelo-osi-tcp-ip`](../07-fundamentos/redes/modelo-osi-tcp-ip.md) — Modelo OSI y Protocolos TCP/IP
- [`windows-arquitectura-ad`](../07-fundamentos/sistemas/windows-arquitectura-ad.md) — Arquitectura de Windows y Active Directory

## T1003.001 (1)

- [`credential-dumping`](../05-post-explotacion/privilege-escalation/windows/credential-dumping.md) — Credential Dumping (Volcado de Credenciales)

## T1003.002 (1)

- [`credential-dumping`](../05-post-explotacion/privilege-escalation/windows/credential-dumping.md) — Credential Dumping (Volcado de Credenciales)

## T1007 (1)

- [`enumeracion-smb`](../02-enumeracion/red/enumeracion-smb.md) — Enumeración SMB (Server Message Block)

## T1014 (1)

- [`volatility-memoria`](../08-forense-dfir/analisis-forense/volatility-memoria.md) — Análisis Forense de Memoria

## T1018 (2)

- [`descubrimiento-hosts`](../01-reconocimiento/activo/descubrimiento-hosts.md) — Descubrimiento de Hosts
- [`enumeracion-ldap`](../02-enumeracion/servicios/enumeracion-ldap.md) — Enumeración LDAP (Lightweight Directory Access Protocol)

## T1021 (2)

- [`enumeracion-rabbitmq`](../02-enumeracion/servicios/enumeracion-rabbitmq.md) — Enumeración RabbitMQ (Puerto 5672 / 15672)
- [`enumeracion-telnet`](../02-enumeracion/servicios/enumeracion-telnet.md) — Enumeración Telnet (Puerto 23)

## T1021.001 (2)

- [`ejecucion-remota-windows`](../05-post-explotacion/lateral-movement/ejecucion-remota-windows.md) — Ejecución Remota en Windows
- [`enumeracion-rdp`](../02-enumeracion/servicios/enumeracion-rdp.md) — Enumeración RDP (Remote Desktop Protocol)

## T1021.002 (1)

- [`ejecucion-remota-windows`](../05-post-explotacion/lateral-movement/ejecucion-remota-windows.md) — Ejecución Remota en Windows

## T1021.005 (1)

- [`enumeracion-vnc`](../02-enumeracion/servicios/enumeracion-vnc.md) — Enumeración VNC (Virtual Network Computing) (Puerto 5900+)

## T1021.006 (3)

- [`ejecucion-remota-windows`](../05-post-explotacion/lateral-movement/ejecucion-remota-windows.md) — Ejecución Remota en Windows
- [`empire-framework`](../06-frameworks-herramientas/powershell-empire/empire-framework.md) — Post-Explotación y C2
- [`enumeracion-winrm`](../02-enumeracion/servicios/enumeracion-winrm.md) — Enumeración WinRM (Windows Remote Management)

## T1027 (1)

- [`analisis-estatico-dinamico`](../08-forense-dfir/malware-analysis/analisis-estatico-dinamico.md) — Estático y Dinámico

## T1041 (1)

- [`transferencia-archivos`](../05-post-explotacion/exfiltracion/transferencia-archivos.md) — Transferencia de Archivos en Post-Explotación

## T1046 (21)

- [`banner-grabbing-http`](../02-enumeracion/web/banner-grabbing-http.md) — Banner Grabbing HTTP
- [`enumeracion-docker`](../02-enumeracion/servicios/enumeracion-docker.md) — Enumeración Docker (Puerto 2375/2376)
- [`enumeracion-elasticsearch`](../02-enumeracion/servicios/enumeracion-elasticsearch.md) — Enumeración Elasticsearch (Puerto 9200)
- [`enumeracion-ftp`](../02-enumeracion/servicios/enumeracion-ftp.md) — Enumeración FTP (File Transfer Protocol)
- [`enumeracion-http-nmap`](../02-enumeracion/web/enumeracion-http-nmap.md) — Enumeración HTTP con Scripts NSE de Nmap
- [`enumeracion-ipmi`](../02-enumeracion/red/enumeracion-ipmi.md) — Enumeración IPMI (Intelligent Platform Management Interface) (Puerto 623/UDP)
- [`enumeracion-memcached`](../02-enumeracion/servicios/enumeracion-memcached.md) — Enumeración Memcached (Puerto 11211)
- [`enumeracion-mongodb`](../02-enumeracion/servicios/enumeracion-mongodb.md) — Enumeración MongoDB (Puerto 27017)
- [`enumeracion-mssql`](../02-enumeracion/servicios/enumeracion-mssql.md) — Enumeración MSSQL (Microsoft SQL Server)
- [`enumeracion-mysql`](../02-enumeracion/servicios/enumeracion-mysql.md) — Enumeración MySQL
- [`enumeracion-oracle`](../02-enumeracion/servicios/enumeracion-oracle.md) — Enumeración Oracle Database
- [`enumeracion-postgresql`](../02-enumeracion/servicios/enumeracion-postgresql.md) — Enumeración PostgreSQL (Puerto 5432)
- [`enumeracion-redis`](../02-enumeracion/servicios/enumeracion-redis.md) — Enumeración Redis (Puerto 6379)
- [`enumeracion-smtp`](../02-enumeracion/servicios/enumeracion-smtp.md) — Enumeración SMTP (Simple Mail Transfer Protocol)
- [`enumeracion-snmp`](../02-enumeracion/red/enumeracion-snmp.md) — Enumeración SNMP (Simple Network Management Protocol)
- [`enumeracion-ssh`](../02-enumeracion/servicios/enumeracion-ssh.md) — Enumeración SSH (Secure Shell)
- [`enumeracion-telnet`](../02-enumeracion/servicios/enumeracion-telnet.md) — Enumeración Telnet (Puerto 23)
- [`enumeracion-webdav`](../02-enumeracion/web/enumeracion-webdav.md) — Enumeración y Prueba de WebDAV
- [`escaneo-puertos`](../01-reconocimiento/activo/escaneo-puertos.md) — Escaneo de Puertos
- [`fingerprinting-os-servicios`](../01-reconocimiento/activo/fingerprinting-os-servicios.md) — Fingerprinting de OS y Servicios
- [`fuzzing-lfi-ssrf`](../02-enumeracion/fuzzing/fuzzing-lfi-ssrf.md) — Fuzzing de LFI y Escaneo de Puertos vía SSRF

## T1047 (1)

- [`ejecucion-remota-windows`](../05-post-explotacion/lateral-movement/ejecucion-remota-windows.md) — Ejecución Remota en Windows

## T1048.001 (1)

- [`canales-encubiertos`](../05-post-explotacion/exfiltracion/canales-encubiertos.md) — Exfiltración por Canales Encubiertos

## T1048.003 (1)

- [`canales-encubiertos`](../05-post-explotacion/exfiltracion/canales-encubiertos.md) — Exfiltración por Canales Encubiertos

## T1053.003 (2)

- [`abuso-cron-jobs`](../05-post-explotacion/privilege-escalation/linux/abuso-cron-jobs.md) — Abuso de Cron Jobs
- [`persistencia-linux`](../05-post-explotacion/persistencia/persistencia-linux.md) — Persistencia en Linux

## T1053.005 (2)

- [`abuso-tareas-programadas`](../05-post-explotacion/privilege-escalation/windows/abuso-tareas-programadas.md) — Abuso de Tareas Programadas (Scheduled Tasks)
- [`persistencia-windows`](../05-post-explotacion/persistencia/persistencia-windows.md) — Persistencia en Windows

## T1055 (3)

- [`explotacion-process-injection`](../04-explotacion/client-side/explotacion-process-injection.md) — Process Injection y Evasión
- [`volatility-memoria`](../08-forense-dfir/analisis-forense/volatility-memoria.md) — Análisis Forense de Memoria
- [`windows-api-hacking`](../07-fundamentos/sistemas/windows-api-hacking.md) — Windows API para Hacking

## T1055.012 (1)

- [`explotacion-process-injection`](../04-explotacion/client-side/explotacion-process-injection.md) — Process Injection y Evasión

## T1059 (2)

- [`analisis-estatico-dinamico`](../08-forense-dfir/malware-analysis/analisis-estatico-dinamico.md) — Estático y Dinámico
- [`metasploit-avanzado`](../06-frameworks-herramientas/metasploit/metasploit-avanzado.md) — Uso Avanzado y Automatización

## T1059.001 (1)

- [`empire-framework`](../06-frameworks-herramientas/powershell-empire/empire-framework.md) — Post-Explotación y C2

## T1068 (5)

- [`analisis-parches-windows`](../03-analisis-vulnerabilidades/sistema/analisis-parches-windows.md) — Gestión de Parches en Windows
- [`analisis-permisos-linux`](../03-analisis-vulnerabilidades/sistema/analisis-permisos-linux.md) — Permisos Inseguros en Linux
- [`explotacion-kernel-linux`](../04-explotacion/sistema/explotacion-kernel-linux.md) — Explotación de Kernel Linux
- [`explotacion-kernel-windows`](../04-explotacion/sistema/explotacion-kernel-windows.md) — Explotación de Kernel Windows
- [`kernel-exploits-privesc-linux`](../05-post-explotacion/privilege-escalation/linux/kernel-exploits-privesc-linux.md) — Escalada de Privilegios mediante Kernel Exploits (Linux)

## T1069 (2)

- [`bloodhound`](../06-frameworks-herramientas/otros/bloodhound.md) — Análisis de Rutas de Ataque en AD
- [`enumeracion-smb`](../02-enumeracion/red/enumeracion-smb.md) — Enumeración SMB (Server Message Block)

## T1069.002 (2)

- [`bloodhound`](../06-frameworks-herramientas/otros/bloodhound.md) — Análisis de Rutas de Ataque en AD
- [`enumeracion-ldap`](../02-enumeracion/servicios/enumeracion-ldap.md) — Enumeración LDAP (Lightweight Directory Access Protocol)

## T1071.004 (1)

- [`canales-encubiertos`](../05-post-explotacion/exfiltracion/canales-encubiertos.md) — Exfiltración por Canales Encubiertos

## T1078 (1)

- [`analisis-servicios-mal-configurados`](../03-analisis-vulnerabilidades/sistema/analisis-servicios-mal-configurados.md) — Servicios Mal Configurados

## T1082 (2)

- [`enumeracion-privesc-linux`](../05-post-explotacion/privilege-escalation/linux/enumeracion-privesc-linux.md) — Enumeración para Escalada de Privilegios en Linux
- [`enumeracion-privesc-windows`](../05-post-explotacion/privilege-escalation/windows/enumeracion-privesc-windows.md) — Enumeración para Escalada de Privilegios en Windows

## T1083 (2)

- [`enumeracion-privesc-linux`](../05-post-explotacion/privilege-escalation/linux/enumeracion-privesc-linux.md) — Enumeración para Escalada de Privilegios en Linux
- [`enumeracion-privesc-windows`](../05-post-explotacion/privilege-escalation/windows/enumeracion-privesc-windows.md) — Enumeración para Escalada de Privilegios en Windows

## T1087 (2)

- [`bloodhound`](../06-frameworks-herramientas/otros/bloodhound.md) — Análisis de Rutas de Ataque en AD
- [`enumeracion-smb`](../02-enumeracion/red/enumeracion-smb.md) — Enumeración SMB (Server Message Block)

## T1087.002 (2)

- [`bloodhound`](../06-frameworks-herramientas/otros/bloodhound.md) — Análisis de Rutas de Ataque en AD
- [`enumeracion-ldap`](../02-enumeracion/servicios/enumeracion-ldap.md) — Enumeración LDAP (Lightweight Directory Access Protocol)

## T1090 (1)

- [`pivoting-tunneling`](../05-post-explotacion/lateral-movement/pivoting-tunneling.md) — Pivoting y Tunneling

## T1090.001 (1)

- [`pivoting-tunneling`](../05-post-explotacion/lateral-movement/pivoting-tunneling.md) — Pivoting y Tunneling

## T1098.004 (1)

- [`persistencia-linux`](../05-post-explotacion/persistencia/persistencia-linux.md) — Persistencia en Linux

## T1105 (1)

- [`transferencia-archivos`](../05-post-explotacion/exfiltracion/transferencia-archivos.md) — Transferencia de Archivos en Post-Explotación

## T1106 (1)

- [`windows-api-hacking`](../07-fundamentos/sistemas/windows-api-hacking.md) — Windows API para Hacking

## T1110 (2)

- [`enumeracion-oracle`](../02-enumeracion/servicios/enumeracion-oracle.md) — Enumeración Oracle Database
- [`enumeracion-smb`](../02-enumeracion/red/enumeracion-smb.md) — Enumeración SMB (Server Message Block)

## T1110.001 (3)

- [`enumeracion-ftp`](../02-enumeracion/servicios/enumeracion-ftp.md) — Enumeración FTP (File Transfer Protocol)
- [`enumeracion-ssh`](../02-enumeracion/servicios/enumeracion-ssh.md) — Enumeración SSH (Secure Shell)
- [`explotacion-brute-force-advanced`](../04-explotacion/credenciales/explotacion-brute-force-advanced.md) — Ataques de Fuerza Bruta Avanzados

## T1110.002 (1)

- [`explotacion-hash-cracking`](../04-explotacion/credenciales/explotacion-hash-cracking.md) — Crackeo de Hashes (Metodología)

## T1110.003 (2)

- [`explotacion-brute-force-advanced`](../04-explotacion/credenciales/explotacion-brute-force-advanced.md) — Ataques de Fuerza Bruta Avanzados
- [`explotacion-password-spraying`](../04-explotacion/credenciales/explotacion-password-spraying.md) — Password Spraying

## T1134.001 (1)

- [`token-impersonation`](../05-post-explotacion/privilege-escalation/windows/token-impersonation.md) — Token Impersonation (Suplantación de Tokens)

## T1135 (2)

- [`enumeracion-nfs`](../02-enumeracion/red/enumeracion-nfs.md) — Enumeración NFS (Network File System)
- [`enumeracion-smb`](../02-enumeracion/red/enumeracion-smb.md) — Enumeración SMB (Server Message Block)

## T1187 (1)

- [`explotacion-adcs-relay`](../04-explotacion/red/explotacion-adcs-relay.md) — Abuso de ADCS y Ataques de Relay

## T1190 (22)

- [`analisis-command-injection`](../03-analisis-vulnerabilidades/web/analisis-command-injection.md) — Análisis de Command Injection
- [`analisis-cors`](../03-analisis-vulnerabilidades/web/analisis-cors.md) — Análisis de CORS (Cross-Origin Resource Sharing)
- [`analisis-csrf`](../03-analisis-vulnerabilidades/web/analisis-csrf.md) — Cross-Site Request Forgery (CSRF)
- [`analisis-deserialization`](../03-analisis-vulnerabilidades/web/analisis-deserialization.md) — Análisis de Insecure Deserialization
- [`analisis-idor`](../03-analisis-vulnerabilidades/web/analisis-idor.md) — Insecure Direct Object Reference (IDOR)
- [`analisis-lfi-rfi`](../03-analisis-vulnerabilidades/web/analisis-lfi-rfi.md) — LFI y RFI
- [`analisis-open-redirect`](../03-analisis-vulnerabilidades/web/analisis-open-redirect.md) — Análisis de Open Redirect
- [`analisis-sqli`](../03-analisis-vulnerabilidades/web/analisis-sqli.md) — SQL Injection (SQLi)
- [`analisis-ssrf`](../03-analisis-vulnerabilidades/web/analisis-ssrf.md) — Análisis de SSRF (Server-Side Request Forgery)
- [`analisis-ssti`](../03-analisis-vulnerabilidades/web/analisis-ssti.md) — Análisis de SSTI (Server-Side Template Injection)
- [`analisis-xss`](../03-analisis-vulnerabilidades/web/analisis-xss.md) — Cross-Site Scripting (XSS)
- [`analisis-xxe`](../03-analisis-vulnerabilidades/web/analisis-xxe.md) — Análisis de XXE (XML External Entity)
- [`configuracion-uso-avanzado`](../06-frameworks-herramientas/burp-suite/configuracion-uso-avanzado.md) — Configuración y Uso Avanzado
- [`enumeracion-webdav`](../02-enumeracion/web/enumeracion-webdav.md) — Enumeración y Prueba de WebDAV
- [`explotacion-auth-bypass-oauth`](../04-explotacion/web/explotacion-auth-bypass-oauth.md) — Bypass de Autenticación en OAuth2 y OpenID
- [`explotacion-cms-wordpress`](../04-explotacion/web/explotacion-cms-wordpress.md) — Explotación de CMS WordPress
- [`explotacion-deserialization`](../04-explotacion/web/explotacion-deserialization.md) — Explotación de Deserialización Insegura
- [`explotacion-fileupload`](../04-explotacion/web/explotacion-fileupload.md) — File Upload Exploitation
- [`explotacion-jwt`](../04-explotacion/web/explotacion-jwt.md) — Explotación de JSON Web Tokens (JWT)
- [`explotacion-nosqli`](../04-explotacion/web/explotacion-nosqli.md) — Inyección NoSQL (NoSQLi)
- [`explotacion-shellshock`](../04-explotacion/sistema/explotacion-shellshock.md) — Shellshock (CVE-2014-6271)
- [`explotacion-sqli`](../04-explotacion/web/explotacion-sqli.md) — Inyección SQL (SQLi)

## T1203 (1)

- [`explotacion-buffer-overflow-stack`](../04-explotacion/sistema/explotacion-buffer-overflow-stack.md) — Explotación de Buffer Overflow (Stack-based x86)

## T1210 (4)

- [`explotacion-eternalblue`](../04-explotacion/sistema/explotacion-eternalblue.md) — EternalBlue (MS17-010)
- [`explotacion-kernel-windows`](../04-explotacion/sistema/explotacion-kernel-windows.md) — Explotación de Kernel Windows
- [`explotacion-zerologon`](../04-explotacion/sistema/explotacion-zerologon.md) — Explotación de Zerologon (CVE-2020-1472)
- [`metasploit-avanzado`](../06-frameworks-herramientas/metasploit/metasploit-avanzado.md) — Uso Avanzado y Automatización

## T1211 (1)

- [`explotacion-buffer-overflow-stack`](../04-explotacion/sistema/explotacion-buffer-overflow-stack.md) — Explotación de Buffer Overflow (Stack-based x86)

## T1484 (1)

- [`bloodhound`](../06-frameworks-herramientas/otros/bloodhound.md) — Análisis de Rutas de Ataque en AD

## T1505.003 (1)

- [`web-shells`](../05-post-explotacion/persistencia/web-shells.md) — Web Shells

## T1543.003 (1)

- [`persistencia-windows`](../05-post-explotacion/persistencia/persistencia-windows.md) — Persistencia en Windows

## T1546.004 (1)

- [`persistencia-linux`](../05-post-explotacion/persistencia/persistencia-linux.md) — Persistencia en Linux

## T1547.001 (1)

- [`persistencia-windows`](../05-post-explotacion/persistencia/persistencia-windows.md) — Persistencia en Windows

## T1547.006 (1)

- [`persistencia-linux`](../05-post-explotacion/persistencia/persistencia-linux.md) — Persistencia en Linux

## T1548.001 (4)

- [`capabilities-linux`](../05-post-explotacion/privilege-escalation/linux/capabilities-linux.md) — Explotación de Linux Capabilities
- [`enumeracion-nfs`](../02-enumeracion/red/enumeracion-nfs.md) — Enumeración NFS (Network File System)
- [`nfs-no-root-squash`](../05-post-explotacion/privilege-escalation/linux/nfs-no-root-squash.md) — Escalada de Privilegios vía NFS no_root_squash
- [`suid-sgid`](../05-post-explotacion/privilege-escalation/linux/suid-sgid.md) — Explotación de Binarios SUID/SGID

## T1548.002 (1)

- [`empire-framework`](../06-frameworks-herramientas/powershell-empire/empire-framework.md) — Post-Explotación y C2

## T1548.003 (1)

- [`abuso-sudo`](../05-post-explotacion/privilege-escalation/linux/abuso-sudo.md) — Abuso de Configuraciones Sudo

## T1550.002 (1)

- [`pass-the-hash`](../05-post-explotacion/lateral-movement/pass-the-hash.md) — Pass-the-Hash (PtH)

## T1550.003 (1)

- [`pass-the-ticket`](../05-post-explotacion/lateral-movement/pass-the-ticket.md) — Pass-the-Ticket

## T1557.001 (4)

- [`explotacion-adcs-relay`](../04-explotacion/red/explotacion-adcs-relay.md) — Abuso de ADCS y Ataques de Relay
- [`explotacion-mitm-responder`](../04-explotacion/red/explotacion-mitm-responder.md) — Envenenamiento LLMNR/NBT-NS con Responder
- [`explotacion-mitm6`](../04-explotacion/red/explotacion-mitm6.md) — IPv6 DNS Takeover con mitm6
- [`explotacion-smb-relay`](../04-explotacion/red/explotacion-smb-relay.md) — SMB Relay Attack

## T1557.002 (1)

- [`explotacion-arp-spoofing`](../04-explotacion/red/explotacion-arp-spoofing.md) — ARP Spoofing y Envenenamiento de Red

## T1558 (1)

- [`enumeracion-kerberos`](../02-enumeracion/red/enumeracion-kerberos.md) — Enumeración Kerberos (Puerto 88)

## T1558.001 (1)

- [`golden-ticket`](../05-post-explotacion/lateral-movement/golden-ticket.md) — Golden Ticket

## T1558.002 (1)

- [`silver-ticket`](../05-post-explotacion/lateral-movement/silver-ticket.md) — Silver Ticket

## T1558.003 (2)

- [`enumeracion-kerberos`](../02-enumeracion/red/enumeracion-kerberos.md) — Enumeración Kerberos (Puerto 88)
- [`explotacion-kerberoasting`](../04-explotacion/credenciales/explotacion-kerberoasting.md) — Kerberoasting y AS-REP Roasting

## T1558.004 (2)

- [`enumeracion-kerberos`](../02-enumeracion/red/enumeracion-kerberos.md) — Enumeración Kerberos (Puerto 88)
- [`explotacion-kerberoasting`](../04-explotacion/credenciales/explotacion-kerberoasting.md) — Kerberoasting y AS-REP Roasting

## T1566 (1)

- [`explotacion-phishing-tecnico`](../04-explotacion/client-side/explotacion-phishing-tecnico.md) — Phishing Técnico (Evilginx2 & Gophish)

## T1572 (1)

- [`pivoting-tunneling`](../05-post-explotacion/lateral-movement/pivoting-tunneling.md) — Pivoting y Tunneling

## T1574.001 (1)

- [`abuso-servicios-windows`](../05-post-explotacion/privilege-escalation/windows/abuso-servicios-windows.md) — Abuso de Servicios de Windows

## T1574.010 (1)

- [`abuso-servicios-windows`](../05-post-explotacion/privilege-escalation/windows/abuso-servicios-windows.md) — Abuso de Servicios de Windows

## T1583 (1)

- [`analisis-estatico-dinamico`](../08-forense-dfir/malware-analysis/analisis-estatico-dinamico.md) — Estático y Dinámico

## T1587.001 (1)

- [`explotacion-msfvenom`](../04-explotacion/client-side/explotacion-msfvenom.md) — Generación de Payloads con msfvenom

## T1589.002 (2)

- [`enumeracion-smtp`](../02-enumeracion/servicios/enumeracion-smtp.md) — Enumeración SMTP (Simple Mail Transfer Protocol)
- [`recoleccion-emails`](../01-reconocimiento/pasivo/recoleccion-emails.md) — Recolección de Emails

## T1590.002 (2)

- [`dns-pasivo`](../01-reconocimiento/pasivo/dns-pasivo.md) — DNS Pasivo
- [`enumeracion-dns`](../02-enumeracion/red/enumeracion-dns.md) — Enumeración DNS (Domain Name System)

## T1592.002 (3)

- [`fingerprinting-os-servicios`](../01-reconocimiento/activo/fingerprinting-os-servicios.md) — Fingerprinting de OS y Servicios
- [`fingerprinting-tecnologias-web`](../01-reconocimiento/pasivo/fingerprinting-tecnologias-web.md) — Fingerprinting Pasivo de Tecnologías Web
- [`fingerprinting-tecnologias-web-activo`](../02-enumeracion/web/fingerprinting-tecnologias-web-activo.md) — Fingerprinting Activo de Tecnologías Web

## T1593 (2)

- [`google-dorking`](../01-reconocimiento/pasivo/google-dorking.md) — Google Dorking
- [`wayback-machine`](../01-reconocimiento/pasivo/wayback-machine.md) — Wayback Machine

## T1595 (1)

- [`deteccion-waf`](../01-reconocimiento/activo/deteccion-waf.md) — Detección de WAF (Web Application Firewall)

## T1595.001 (1)

- [`descubrimiento-hosts`](../01-reconocimiento/activo/descubrimiento-hosts.md) — Descubrimiento de Hosts

## T1595.002 (13)

- [`analisis-nessus`](../03-analisis-vulnerabilidades/scanning/analisis-nessus.md) — Análisis de Vulnerabilidades con Nessus
- [`analisis-nmap-nse`](../03-analisis-vulnerabilidades/scanning/analisis-nmap-nse.md) — Análisis de Vulnerabilidades con Nmap Scripting Engine (NSE)
- [`analisis-seguridad-cabeceras`](../03-analisis-vulnerabilidades/web/analisis-seguridad-cabeceras.md) — Seguridad de Cabeceras HTTP
- [`analisis-software-obsoleto`](../03-analisis-vulnerabilidades/sistema/analisis-software-obsoleto.md) — Software Obsoleto
- [`configuracion-uso-avanzado`](../06-frameworks-herramientas/burp-suite/configuracion-uso-avanzado.md) — Configuración y Uso Avanzado
- [`escaneo-openvas-gvm`](../03-analisis-vulnerabilidades/scanning/escaneo-openvas-gvm.md) — Escaneo de Vulnerabilidades con OpenVAS / GVM
- [`escaneo-vulnerabilidades`](../01-reconocimiento/activo/escaneo-vulnerabilidades.md) — Escaneo de Vulnerabilidades
- [`fuzzing-directorios-archivos`](../02-enumeracion/fuzzing/fuzzing-directorios-archivos.md) — Fuzzing de Directorios y Archivos Web
- [`fuzzing-lfi-ssrf`](../02-enumeracion/fuzzing/fuzzing-lfi-ssrf.md) — Fuzzing de LFI y Escaneo de Puertos vía SSRF
- [`fuzzing-parametros`](../02-enumeracion/fuzzing/fuzzing-parametros.md) — Fuzzing de Parámetros HTTP
- [`fuzzing-subdominios-vhosts`](../02-enumeracion/fuzzing/fuzzing-subdominios-vhosts.md) — Fuzzing de Subdominios y Virtual Hosts
- [`metasploit-avanzado`](../06-frameworks-herramientas/metasploit/metasploit-avanzado.md) — Uso Avanzado y Automatización
- [`nikto`](../02-enumeracion/web/nikto.md) — Escaneo de Vulnerabilidades Web con Nikto

## T1596 (1)

- [`shodan-censys`](../01-reconocimiento/pasivo/shodan-censys.md) — Shodan y Censys

## T1596.002 (1)

- [`whois-registros-dominio`](../01-reconocimiento/pasivo/whois-registros-dominio.md) — WHOIS y Registros de Dominio

## T1596.003 (1)

- [`transparencia-certificados`](../01-reconocimiento/pasivo/transparencia-certificados.md) — Transparencia de Certificados (Certificate Transparency)

## T1613 (2)

- [`enumeracion-docker`](../02-enumeracion/servicios/enumeracion-docker.md) — Enumeración Docker (Puerto 2375/2376)
- [`enumeracion-kubernetes`](../02-enumeracion/servicios/enumeracion-kubernetes.md) — Enumeración Kubernetes (K8s) (Puerto 6443 / 443 / 10250)
